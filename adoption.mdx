...
With Engineering System 1.5.1, the user-facing request can be:
...
Engineering System 1.5.1 adopted repositories record enough identity to prove exactly which canonical standard and which project-native gates are in force:
~~~yaml
engineering_system:
  version: "1.5.1"
  mode: adopted
...
Engineering System 1.5 also routes material design-bearing changes through the lightweight `standards/DESIGN.md` gate and production-impacting failures through `standards/OPERATIONS.md`. This does not require a new design document or incident document for every task; the smallest durable artifact that fits the decision remains preferred.
